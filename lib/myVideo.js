...
var myVideo = (function () {
    ...
    // 기본데이터
    var classNameWhenShowingCtrlBar = 'show';
        ...
        var template = 
            '<div class="my-video" style="width:' + width + 'px; height:' + height + 'px;">' +
                ...
                '<div class="controlbar show">' +
                    ...
                    '<div class="cell-wrap">' +
                        '<div class="cell left">' +
                            '<button class="play btn focusable"></button>' +
                        '</div>' +
                        '<div class="cell left">' +
                            '<button class="stop btn focusable"</button>' +
                        '</div>' +
                        '<div class="cell left">' +
                            '<button class="mute btn focusable"></button>' +
                        '</div>' +
                        '<div class="volume left">' +
                            '<input type="range" class="bar focusable" min=0 max=1 step=0.01 value=1>' +
                        '</div>' +
                        '<div class="text cell left">' +
                            '<span class="current-time-text">0:00:00</span> / ' +
                            '<span class="total-time-text">0:00:00</span>' +
                        '</div>' +
                        '<div class="cell right">' +
                            '<button class="full btn focusable"></button>' +
                        '</div>' +
                        '<div class="zoom cell right">' +
                            '<button class="zoom btn focusable"></button>' +
                        '</div>' +
                    '</div>' +
                '</div>' +
    ...
    var hideControlbar = function (vp) { 
        vp.elements.controlbar.classList.remove(classNameWhenShowingCtrlBar);
    };

    var revealControlbar = function (vp) {
        vp.elements.controlbar.classList.add(classNameWhenShowingCtrlBar);
    };
    ...
    var changeVideoFull = function (vp) {
        ...
        if (vp.state.isFull) {
            ...
            vp.videoframe.classList.add('full');
        } else {
            handleTurnOffFullscreen(vp);
            vp.videoframe.classList.remove('full');
        }
    };
    ...
    var installEvents = function (vp) {
        ...
        window.addEventListener("resize", function () {
            if (!vp.state.isFull && vp.state.isZoomIn) { 
                resizeVideo(vp);
            }
        });
        ...
        var handleESCKeyOnIE = function (event) {
            if (vp.videoframe == document.querySelector('.active-element').closest('.my-video')) {
                event.preventDefault();
                fullScreenController.changeFullscreen(vp);
                changeVideoFull(vp);
        ...
        //풀스크린 - IE를 제외한 나머지 브라우저의 esc는 키이벤트가 아니라 여기서 다룸
