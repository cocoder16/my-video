var myVideo = (function () {
    ...
    var resizeVideo = function (vp) {
        ...
        if (vp.video.getBoundingClientRect().height > window.innerHeight*0.8){
            setSize(vp, 'auto', window.innerHeight * 0.8 + 'px');
        }  
    };

    var changeVideoZoom = function (vp) {
        if ((browser.name !== "IE" && !fullScreenController.isFull.other) || (browser.name === "IE" && !fullScreenController.isFull.IE)){
    ...
    var handleTurnOnFullscreen = function (vp) {
        vp.state.isFull = true;
        sizeToFull(vp);
    };

    var handleTurnOffFullscreen = function (vp) {
        vp.state.isFull = false;
    ...
    var changeVideoFull = function (vp) {
        if (fullScreenController.isFull.other || fullScreenController.isFull.IE) {
    ...
    //방치
    var hideMouseWhenIdle = function (vp) { 
        if (timeout) {
            clearTimeout(timeout);
        }
    ...
    var installEvents = function (vp) {
        ...
        window.addEventListener("mouseup", function () { // 진행바, 볼륨바 둘다 포함.
        ...
        window.addEventListener("resize", function () {
            if(fullScreenController.isFull.IE == false){ //IE 풀스크린때 리사이즈 이벤트 발생
                if (vp.state.isZoomIn){
                    resizeVideo(vp);
                }
            }
        ...
        if (browser.name !== 'IE') {
            ...
            document.addEventListener("keydown", function (event) {
                handleKeyDown(event);
                //풀스크린 - esc - ie
                if (event.keyCode == 27 && fullScreenController.isFull.IE) {
                    event.preventDefault();
                    fullScreenController.isFull.IE = false;
                    changeVideoFull(vp);
                }
            });
            document.addEventListener("keyup", function (event) {
                handleKeyUp(vp, event.keyCode);
            });
        }

        //풀스크린 - esc는 키이벤트가 아니라 여기서 다룸
        var handleFullscreenChange = function (event, vp) {
            if (fullScreenController.isFull.other) {
                fullScreenController.isFull.other = false;
            }
            if (fullScreenController.isFull.IE) {
                fullScreenController.isFull.IE = false;
            }
